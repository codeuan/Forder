...
/**
 * Adapter between the JavaFX Cell[][] board and the AI's 64-bit board.
 *
 * RIGHT = 1
 * LEFT  = 0
 */
public final class ForderBoardConverter {
    ...
    public static long encode(Cell[][] cells) {
        validateGrid(cells);
    ...
    private static void validateGrid(Cell[][] cells) {
        if (cells == null || cells.length != ForderRules.ROWS) {
            throw new IllegalArgumentException(
                    "Expected an " + ForderRules.ROWS + "x"
                            + ForderRules.COLS + " Cell array."
            );
        }
